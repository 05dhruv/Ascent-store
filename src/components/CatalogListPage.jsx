    ...
    Object.fromEntries(
      Object.entries(row || {}).map(([key, value]) => {
        ...
        const withoutCurrency = value
          ...
          .replace(/\bRs\.?\s*/gi, "")
          .trim();
        const numericValue = withoutCurrency.replace(/,/g, "");
        return [
          key,
...
function removeTemplateFormulaErrors(worksheet, headers) {
  const maxColumn = headers.length - 1;
  Object.keys(worksheet).forEach((ref) => {
    if (ref.startsWith("!")) return;
    const cellAddress = XLSX.utils.decode_cell(ref);
    const cell = worksheet[ref];
    if (
      cellAddress.c > maxColumn ||
      cell?.f ||
      cell?.v === "#N/A" ||
      cell?.w === "#N/A"
    ) {
  ...
  range.s.r = 0;
  range.e.c = maxColumn;
  range.e.r = Math.max(range.e.r, TEMPLATE_ROW_LIMIT - 1);
  worksheet["!ref"] = XLSX.utils.encode_range(range);
  delete worksheet["!dataValidation"];
...
function optionRangeFormula(optionGroups, key) {
...
export default function CatalogListPage({
  ...
  const [productImportRange, setProductImportRange] = useState(0);
  ...
  const importBulkRows = async (rowsToImport, importOptions = {}) => {
    ...
    try {
      ...
      if (importOptions.file) {
        ...
        formData.append("range", String(importOptions.range || 0));
        formData.append(
          "selectedIndexes",
          JSON.stringify(importOptions.selectedIndexes || []),
        );
  ...
  const closeProductImportPreview = () => {
    ...
    setProductImportRange(0);
  };

  const confirmProductImportPreview = async () => {
    const selectedPreviewRows = productImportPreviewRows.filter(
      (row) => productImportSelected[row.__previewId],
    );
    const selectedRows = selectedPreviewRows.map(
      ({ __previewId, __serialNo, __sourceIndex, ...row }) => row,
    );
    const selectedIndexes = selectedPreviewRows
      .map((row) => Number(row.__sourceIndex))
      .filter(Number.isFinite);
    const file = productImportFile;
    const range = productImportRange;
    closeProductImportPreview();
    await importBulkRows(selectedRows, { file, range, selectedIndexes });
  };
  ...
  const downloadTemplate = async () => {
    try {
      ...
      if (bulkImportType === "products") {
        removeTemplateFormulaErrors(ws, headers);
      }
      const optionGroups =
        bulkImportType === "products"
          ? [
              {
                ...
                values: await fetchOptionNames(
                  "/api/catalog/brands?pageSize=5000",
                ),
              },
              {
                key: "categories",
              ...
              },
              {
                key: "sub_categories",
                values: await fetchOptionNames(
                  "/api/catalog/sub-categories?pageSize=5000",
                ),
              },
              {
                key: "income_heads",
                values: await fetchOptionNames(
                  "/api/catalog/income-heads?pageSize=5000",
                ),
              },
      ...
      if (bulkImportType === "products") {
        addValidation(
          headerIndex("Brand"),
          optionRangeFormula(optionGroups, "brands"),
        );
        addValidation(
          headerIndex("Category"),
          optionRangeFormula(optionGroups, "categories"),
        );
        addValidation(
          headerIndex("Sub Category"),
          optionRangeFormula(optionGroups, "sub_categories"),
        );
        addValidation(
          headerIndex("Income Head"),
          optionRangeFormula(optionGroups, "income_heads"),
        );
        addValidation(
          headerIndex("Opening Stock Store"),
          optionRangeFormula(optionGroups, "stores"),
        );
      }
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const importRange = startsWithRequirementRow ? 1 : 0;
      ...
      if (bulkImportType === "products") {
        ...
        setProductImportRange(importRange);
                ...
                <tbody className="divide-y divide-gray-100">
                  {productImportPreviewRows.map((row) => (
                    <tr key={row.__previewId} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        {row["Image URL"] || row.image_url || row.image ? (
                          <img
                            src={row["Image URL"] || row.image_url || row.image}
                            alt=""
            ...
            <div className="relative">
              ...
              {bulkOpen && (
                <>
                  ...
                  />
                  <div className="absolute left-0 top-full z-[200] mt-1 max-h-[70vh] w-[min(13rem,calc(100vw-2rem))] overflow-y-auto rounded-xl border border-gray-200 bg-white py-1 shadow-lg sm:left-auto sm:right-0">
                    {bulkMenuItems.map((item, i) => (
                      <button
                        ...
                        className="w-full break-words px-4 py-2.5 text-left text-sm text-gray-700 transition-colors hover:bg-gray-50"
                      >
