...
export default function HomePage() {
  ...
  useEffect(() => {
    if (!user) return;
    fetch('/api/catalog/promotions?pageSize=100', { cache: 'no-store' })
      .then((res) => res.json())
      .then((json) => {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const records = json.success ? (json.data?.records || []) : [];
        setPromotionAlerts(records.flatMap((promo) => {
          const status = String(promo.status || '').toLowerCase();
          const end = promo.end_date ? new Date(`${String(promo.end_date).slice(0, 10)}T23:59:59`) : null;
          const daysLeft = end ? Math.ceil((end - today) / 86400000) : null;
          if (status === 'pending') return [{ ...promo, kind: 'Pending approval' }];
          if (status === 'active' && daysLeft !== null && daysLeft >= 0 && daysLeft <= 7) return [{ ...promo, kind: `Ends in ${daysLeft} day${daysLeft === 1 ? '' : 's'}` }];
          if (status === 'active' && end && end < today) return [{ ...promo, kind: 'Expired' }];
          return [];
        }).slice(0, 6));
      })
      .catch(() => setPromotionAlerts([]));
  }, [user]);
  ...
  const dashboardStats = [
    ...
    },
    {
      ...
      valueOverride: `₹${Number(stats.purchaseTotal?.amount || 0).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`,
      value: `₹${Number(stats.vendorPayable?.amount || 0).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`,
      unit: '',
      sub: `${stats.purchaseTotal?.grns || 0} confirmed GRNs · ${stats.dataScopeLabel || 'Assigned stores'}`,
      href: '/purchase/grn',
      subBlue: false,
    },
    {
      icon: 'ti-chart-line',
      label: 'Total Sale',
      value: `₹${Number(stats.sales?.totalRevenue || 0).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`,
      unit: '',
      sub: `${stats.sales?.totalSales || 0} orders · ${stats.dataScopeLabel || 'Assigned stores'}`,
      href: '/sales/pos',
      subBlue: true,
    },
    {
      ...
      valueOverride: `₹${Number(stats.vendorPayable?.amount || 0).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`,
      unit: '',
      sub: `${stats.vendorPayable?.bills || 0} pending vendor bills`,
            ...
            <div className="flex items-baseline gap-1.5">
              {s.green ? (
                ...
                <p className="text-[22px] md:text-[28px] font-black tracking-tight text-slate-900 leading-none">
                  {loading ? '-' : (s.valueOverride ?? s.value)}
                </p>
              )}
      ...
      {promotionAlerts.length > 0 && (
        <div className="mb-5 rounded-2xl border border-indigo-100 bg-indigo-50/70 p-4">
          ...
          <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
            {promotionAlerts.map((promo) => (
              <div key={`${promo.id}:${promo.kind}`} className="flex items-center justify-between gap-3 rounded-xl border border-white bg-white px-3 py-2.5 shadow-sm">
                <div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-800">{promo.name}</p><p className="text-xs text-slate-500">{promo.kind}{promo.end_date ? ` · Ends ${String(promo.end_date).slice(0, 10)}` : ''}</p></div>
                <span className="shrink-0 rounded-full bg-indigo-100 px-2 py-1 text-[10px] font-bold text-indigo-700">{String(promo.status || '').toUpperCase()}</span>
              </div>
            ))}
          </div>
        </div>
