...
function DataTable({
  ...
  const showingFrom =
    hasPagination && totalRows > 0 ? (page - 1) * pageSize + 1 : 0;
  const showingTo =
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      <div
        className={`overflow-x-auto ${scrollBody ? "max-h-[560px] overflow-y-auto" : ""}`}
      >
        <table className="w-full min-w-[980px] border-separate border-spacing-0">
          <thead className={scrollBody ? "sticky top-0 z-20 bg-white" : ""}>
            <tr className="border-b border-gray-100 shadow-[0_1px_0_rgba(243,244,246,1)]">
              {headers.map((header) => (
                <th
                  key={header}
                  className="bg-white px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wide text-gray-500"
                >
                  {header}
                </th>
              ))}
            ...
            {rows.length ? (
              ...
              <tr>
                <td
                  ...
                  className="px-4 py-14 text-center text-[14px] font-medium text-blue-700"
                >
                  {empty}
                </td>
              </tr>
      ...
      <div className="sticky bottom-0 z-10 flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 bg-white px-4 py-3 text-[12px] text-gray-500">
        {hasPagination ? (
          <>
...
function Row({ children }) {
  return (
    <tr className="border-b border-gray-100 hover:bg-blue-50/50">{children}</tr>
  );
}
...
function ActionButton({ children, onClick, tone = "blue" }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`rounded-lg border px-3 py-1.5 text-[12px] font-semibold ${styles}`}
    >
      {children}
    </button>
  );
...
function TextInput(props) {
  return (
    <input
      {...props}
      className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-[13px] text-gray-800 outline-none focus:border-blue-500"
    />
  );
}
  ...
  return (
    <div className="mb-4 grid gap-3 rounded-xl border border-gray-200 bg-white p-4 md:grid-cols-6">
      ...
      <div className="flex items-end">
        <button
          onClick={onFetch}
          className="w-full rounded-lg border border-blue-200 px-4 py-2 text-[13px] font-semibold text-blue-600"
        >
          Fetch
        </button>
      </div>
...
export function VendorQuotationsPage() {
  ...
  const save = async () => {
    ...
    if (!Number(form.qty) || Number(form.qty) <= 0)
      return alert("Qty must be greater than 0");
    if (Number(form.quotedPrice) < 0)
      return alert("Quoted price cannot be negative");
  ...
  return (
    <PageShell
      ...
      description="Capture vendor quotes and compare landed price, lead time and score."
    >
      ...
      <div className="mb-4 grid gap-3 rounded-xl border border-gray-200 bg-white p-4 md:grid-cols-4">
        ...
        <button
          onClick={save}
          className="rounded-lg bg-blue-600 px-4 py-2 text-[13px] font-semibold text-white md:col-span-4"
        >
          Save Quotation
        </button>
      </div>
      <DataTable
        ...
        renderRow={(row) => (
          <Row key={row.id}>
            ...
            <Cell>{row.storeName}</Cell>
            <Cell>{row.totalItems}</Cell>
            <Cell>{money(row.totalAmount)}</Cell>
            <Cell>{row.deliveryDays} days</Cell>
            <Cell>{money(row.freightAmount)}</Cell>
            <Cell>{Math.round(row.score)}</Cell>
...
export function PurchaseReturnsPage() {
  ...
  const load = useCallback(
    () =>
      fetch(`/api/purchase/returns${buildQuery({ search, ...filters })}`, {
        credentials: "include",
        cache: "no-store",
      })
        .then((r) => r.json())
  ...
  return (
    <PageShell
      ...
      description="Record stock returned to vendors and track return value."
    >
      <Toolbar search={search} setSearch={setSearch}>
        <button
          onClick={() => downloadCsv("purchase-returns.csv", filtered)}
          className="rounded-lg border border-gray-200 px-4 py-2 text-[13px] font-semibold text-gray-700"
        >
          Export CSV
        </button>
      ...
      <div className="mb-4 grid gap-3 rounded-xl border border-gray-200 bg-white p-4 md:grid-cols-4">
        ...
        <button
          onClick={save}
          className="rounded-lg bg-blue-600 px-4 py-2 text-[13px] font-semibold text-white"
        >
          Save Return
        </button>
      </div>
...
export function VendorLedgerPage() {
  ...
  const load = useCallback(
    () =>
      fetch(
        `/api/purchase/vendor-ledger${buildQuery({ search, ...filters })}`,
  ...
  useEffect(() => {
    setPage(1);
  }, [search, filters.dateFrom, filters.dateTo, filters.vendorId]);
  useEffect(() => {
  ...
  return (
    <PageShell
      title="Vendor Ledger"
      description="Invoice, payment and purchase return ledger by vendor."
    >
      <Toolbar search={search} setSearch={setSearch}>
        <button
          onClick={() => downloadCsv("vendor-ledger.csv", filtered)}
          className="rounded-lg border border-gray-200 px-4 py-2 text-[13px] font-semibold text-gray-700"
        >
          Export CSV
        </button>
      ...
      <DataTable
        ...
        renderRow={(row, index) => (
          <Row key={`${row.transactionId}-${index}`}>
            <Cell>{dateText(row.entryAt)}</Cell>
            <Cell>{row.vendorName}</Cell>
            <Cell>{row.entryType}</Cell>
            <Cell>{row.transactionId}</Cell>
            <Cell>{row.referenceNo}</Cell>
            <Cell>{money(row.debit)}</Cell>
            <Cell>{money(row.credit)}</Cell>
...
export function VendorPerformancePage() {
  ...
  const load = useCallback(
    () =>
      fetch(
        `/api/purchase/vendor-performance${buildQuery({ search, ...filters })}`,
        { credentials: "include", cache: "no-store" },
      )
        .then((r) => r.json())
        .then((data) => setRows(normalizeList(data)))
  ...
  return (
    <PageShell
      ...
      description="Vendor score based on purchase volume, returns, lead time and outstanding risk."
    >
      <Toolbar search={search} setSearch={setSearch}>
        <button
          onClick={() => downloadCsv("vendor-performance.csv", filtered)}
          className="rounded-lg border border-gray-200 px-4 py-2 text-[13px] font-semibold text-gray-700"
        >
          Export CSV
        </button>
      ...
      <DataTable
        ...
        renderRow={(row) => (
          <Row key={row.vendorId}>
            ...
            <Cell>{row.score}</Cell>
            <Cell>{row.grade}</Cell>
            <Cell>{row.poCount}</Cell>
            <Cell>{money(row.purchaseValue)}</Cell>
            <Cell>{row.avgLeadDays}</Cell>
            <Cell>{row.returnCount}</Cell>
            <Cell>{money(row.outstanding)}</Cell>
          </Row>
        )}
      />
    </PageShell>
...
export function AutoReorderPage() {
  ...
  const load = useCallback(() => {
    ...
    fetch(`/api/purchase/reorder?${params.toString()}`, {
      credentials: "include",
      cache: "no-store",
    })
      .then((r) => r.json())
  ...
  const toggle = (row) =>
    setSelected((current) =>
      current.some(
        (item) =>
          item.variantKey === row.variantKey && item.storeId === row.storeId,
      )
        ? current.filter(
            (item) =>
              !(
  ...
  return (
    <PageShell
      title="Auto Reorder Generation"
      description="Find low-stock products and generate purchase orders from reorder suggestions."
    >
      <Toolbar search={search} setSearch={setSearch}>
        <SelectInput
          value={storeId}
        ...
        <button
          onClick={load}
          className="rounded-lg border border-blue-200 px-4 py-2 text-[13px] font-semibold text-blue-600"
        >
          Fetch
        </button>
        <button
          onClick={() => downloadCsv("auto-reorder.csv", visible)}
          className="rounded-lg border border-gray-200 px-4 py-2 text-[13px] font-semibold text-gray-700"
        >
          Export CSV
        </button>
      ...
      <DataTable
        ...
        renderRow={(row) => {
          ...
          return (
            <Row key={`${row.storeId}-${row.variantKey}`}>
              <Cell>
                <input
                  type="checkbox"
                  checked={checked}
              ...
              <Cell>{money(row.mrp)}</Cell>
              <Cell>{money(row.sellingPrice)}</Cell>
              <Cell>{row.currentStock}</Cell>
              <Cell>{row.mbq}</Cell>
              <Cell>{row.suggestedQty}</Cell>
              <Cell>{row.lastGrnQty || 0}</Cell>
              <Cell>{Number(row.avgDailySales || 0).toFixed(2)}</Cell>
              <Cell>{row.vendorName || "-"}</Cell>
              <Cell>{money(row.costPrice)}</Cell>
            </Row>
          );
        }}
      />
