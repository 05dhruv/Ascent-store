...
import GrnInspection from '@/components/inventory/GrnInspection';
      ...
      .flatMap((item) => {
        if (isWarehouseDestination) {
          return (item.batches || [])
            .map((batch) => ({
              stock_in_item_id:
                batch.stock_in_item_id || item.stock_in_item_id || null,
              product_id: item.product_id,
    ...
    <MainLayout>
      ...
      <div className="flex h-[calc(100vh-132px)] min-h-[520px] flex-col overflow-hidden">
              ...
              <div className="grid min-h-0 flex-1 grid-rows-[auto_minmax(260px,1fr)] gap-4 overflow-hidden p-4">
                    ...
                    selectedVendorIds.length === 0 && (
                      <div className="mb-4 rounded-lg border border-dashed border-gray-200 bg-gray-50 px-4 py-8 text-center">
                        ...
                        <p className="mt-1 text-[12px] text-gray-500">
                          Products from the selected vendors will appear here
                          for quick selection.
                        </p>
                      </div>
                    )}
                ...
                {filteredCart.length > 0 ? (
                  isWarehouseDestination ? (
                    ...
                    <div className="min-h-0 overflow-y-auto rounded-lg border border-gray-100 bg-white">
                      <table className="w-full border-separate border-spacing-0">
                        <thead className="sticky top-0 z-20 bg-white shadow-[0_1px_0_rgba(229,231,235,1)]">
                          <tr>
                            <th className="bg-white py-2 px-2 text-left text-[11px] font-bold uppercase tracking-wide text-gray-500">
                              Product
                            </th>
                            <th className="bg-white py-2 px-2 text-left text-[11px] font-bold uppercase tracking-wide text-gray-500">
                              Qty
        ...
        <div className="shrink-0 bg-white border-t border-gray-200 shadow-[0_-2px_8px_rgba(15,23,42,0.06)]">
          {!isConfirmedStockIn && <div className="max-h-[35vh] overflow-auto px-6"><GrnInspection items={cart} onChange={setCart} form={form} onFormChange={setForm} /></div>}
          <div className="flex items-center justify-between px-6 py-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                type="button"
                onClick={confirm}
                disabled={isConfirmedStockIn || confirming || cart.length === 0}
