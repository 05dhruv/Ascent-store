...
export default function MovementTracker() {
  ...
  const cols=tab==='transfers'?['Transfer','Source → Destination','Status','Sent','Accepted','Transit','Damaged / Rejected','Short / Excess','Open cases','ETA']:tab==='buckets'?['Location','Material','Unit','Available','Reserved','Quarantine','Damaged','Rejected','Physical value']:['Date / Time','Location','Material / Batch','Unit','In','Out','Recorded balance','Document','Actor'];
  return <MainLayout><div className="p-4 sm:p-6 space-y-5">
    ...
    {tab==='buckets'&&<p className="text-xs text-slate-600">Current physical stock; date filters apply to transfers and ledger. Reserved includes picked stock. Transit is shown by transfer.</p>}
    ...
    <div className="overflow-auto rounded border"><table className="w-full whitespace-nowrap text-sm"><thead className="bg-slate-100"><tr>{cols.map(c=><th key={c} className="p-3 text-left">{c}</th>)}</tr></thead><tbody>{rows.map(row=><tr key={row.id||`${row.store_id}-${row.product_id}`} className="border-t">
      {(tab==='transfers'?[<button className="text-indigo-700 underline" onClick={()=>open(row.id)} key="open">{row.transaction_id}</button>,`${row.source_name} → ${row.destination_name}`,row.workflow_version===2?pretty(row.status):`Historical: ${pretty(row.status)}`,row.workflow_version===2?num(row.dispatched):'—',row.workflow_version===2?num(row.accepted):'—',row.workflow_version===2?num(row.in_transit):'—',`${num(row.damaged)} / ${num(row.rejected)}`,`${num(row.short)} / ${num(row.excess)}`,row.open_cases,row.expected_arrival_at?new Date(row.expected_arrival_at).toLocaleString():'—']:tab==='buckets'?[row.location,row.product,row.unit,num(row.available),num(row.reserved),num(row.quarantine),num(row.damaged),num(row.rejected),num(row.physical_value)]:[new Date(row.created_at).toLocaleString(),row.location,`${row.product} / ${row.batch_no||'—'}`,row.unit,row.direction==='in'?num(row.qty):'—',row.direction==='out'?num(row.qty):'—',num(row.recorded_balance),`${pretty(row.reference_type)} ${row.meta?.transactionId||row.reference_id}`,row.actor]).map((cell,i)=><td key={i} className="p-3">{cell}</td>)}
    ...
    {detail&&<section className="rounded-xl border-2 border-indigo-200 bg-white p-4 space-y-4" aria-label="Transfer detail">
      <div className="flex justify-between"><div><h2 className="text-xl font-semibold">{detail.transfer.transaction_id} · {pretty(state)}</h2><p>{detail.transfer.source_name} → {detail.transfer.destination_name}</p></div><button onClick={()=>setDetail(null)}>Close detail</button></div>
      {detail.transfer.workflow_version!==2?<p>Historical transfer. Its original stock posting is preserved; receipt quantities were not captured in this workflow.</p>:<>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {detail.canReceive&&receiving&&<><input aria-label="Excess approval event ID" className="rounded border p-2" placeholder="Excess approval ID (if needed)" value={form.excessApprovalId||''} onChange={e=>setForm({...form,excessApprovalId:e.target.value})}/><button disabled={busy} className={button} onClick={()=>action('receive')}>Confirm actual receipt</button></>}
          {detail.canSend&&receiving&&<button disabled={busy} className={button} onClick={()=>action('approve_excess',{items:lines.filter(x=>Number(x.excess)>0).map(x=>({id:x.id,qty:x.excess}))})}>Approve entered excess</button>}
