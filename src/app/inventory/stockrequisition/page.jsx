...
function productLabel(product) {
  ...
  const meta = [sku, barcode].filter(Boolean).join(" | ");
  return meta
    ? `${product.name} (${meta})`
        ...
        {row.approvalStatus === "pending" && (
          <>
            <button
              ...
              className="rounded border border-green-200 px-2 py-1 text-[11px] font-semibold text-green-700 hover:bg-green-50"
            >
              Approve
            </button>
            <button
          ...
          row.fulfillmentStatus !== "completed" && (
            <>
              ...
              >
                ...
                {stores
                  .filter(
                    (store) => String(store.id) !== String(row.destinationId),
                  )
                  .map((store) => (
              ...
              <div className="mt-5 rounded-xl border border-gray-200">
                ...
                <div className="divide-y divide-gray-100">
                  {form.items.map((item, index) => (
                    <div
                      key={index}
                      className="grid gap-3 px-4 py-3 md:grid-cols-[1fr_120px_40px]"
                    >
                      <div className="relative">
                        <input
                          ...
                          placeholder="Search product by name, SKU, barcode, or ID"
                          className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-red-300 focus:ring-2 focus:ring-red-100"
                        />
                        {activeProductPicker === index && (
                          <div className="absolute left-0 right-0 top-full z-20 mt-1 max-h-64 overflow-auto rounded-lg border border-gray-200 bg-white shadow-xl">
                            {getProductOptions(item).length ? (
                              getProductOptions(item).map((product) => (
                                <button
                                  ...
                                  className="block w-full border-b border-gray-100 px-3 py-2 text-left text-sm hover:bg-red-50"
                                >
