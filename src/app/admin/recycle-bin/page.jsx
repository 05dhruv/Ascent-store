...
function deletedByLabel(item) {
  const id = item.deleted_by_user_id || item.deleted_by || "";
  const email = item.deleted_by_email || "";
  const name = item.deleted_by_name || email || (id ? `User ${id}` : "-");
  ...
  async function runAction(id, type) {
    ...
    try {
      const res = await fetch(
        type === "restore"
          ? `/api/admin/recycle-bin/${id}/restore`
          : `/api/admin/recycle-bin/${id}`,
  ...
  async function runBulkAction(type) {
    ...
    try {
      const res = await fetch("/api/admin/recycle-bin/bulk", {
        ...
        body: JSON.stringify({
          action: type === "restore" ? "restore" : "purge",
          ids,
        }),
      });
      const json = await res.json().catch(() => ({}));
              ...
              <div className="flex gap-4 px-7 py-6">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-500">
                    {confirmAction.type === "bulk-restore" ? (
                      <>
                        Selected records and their related rows will be restored
                        together.
                      </>
                    ) : confirmAction.type === "bulk-purge" ? (
                      <>
                        Selected records and their related rows will be
                      ...
                      <>
                        ...
                        will be restored. Related rows from the same delete
                        operation may also be restored.
                      </>
                    ) : confirmAction.type === "purge-expired" ? (
                      <>
                        All recycle-bin records older than 15 days will be
                        permanently deleted.
                      </>
                    ) : (
                      <>
                        ...
                        will be permanently deleted from recycle bin.
                      </>
                    )}
                  </p>
                </div>
          ...
          <div className="flex flex-col justify-between gap-3 rounded-lg border border-slate-200 bg-white px-4 py-4 shadow-sm md:flex-row md:items-end">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Deleted records are retained for 15 days with restore and delete
                audit history.
              </p>
            </div>
              ...
              <div className="flex flex-col gap-3 border-b border-slate-200 bg-indigo-50 px-4 py-3 text-sm text-indigo-900 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      openConfirm({ type: "bulk-purge", count: selectedCount })
                    }
                    disabled={!!actionLoading}
                        ...
                        >
                          <td className="px-4 py-3">
                            <input
                              ...
                              disabled={
                                item.status !== "deleted" || !!actionLoading
                              }
                          ...
                          <td className="px-4 py-3">
                            ...
                            <div className="mt-1 text-xs text-slate-500">
                              ID: {item.resource_id || "-"} | Fields:{" "}
                              {item.field_count}
                            </div>
                          </td>
                          <td className="whitespace-nowrap px-4 py-3 text-slate-700">
                            {item.type_label || item.table_name}
                          </td>
                          <td className="whitespace-nowrap px-4 py-3 text-slate-700">
      ...
      {false && confirmAction ? (
        <>
          <button
            ...
            onClick={() => setConfirmAction(null)}
          />
          <div className="fixed left-1/2 top-1/2 z-[90] max-h-[calc(100vh-32px)] w-[calc(100vw-32px)] max-w-[536px] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-[18px] bg-white text-left shadow-2xl">
            <div className="flex gap-4 px-7 py-6">
              <div
                className={
                  confirmAction.type === "restore"
