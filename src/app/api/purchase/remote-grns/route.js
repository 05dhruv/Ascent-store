...
function canOpenRemoteGrnDraft(user) {
  return (
    canEditRemoteGrnCp(user) ||
    canEditRemoteGrnSp(user) ||
    canApproveRemoteGrn(user)
  );
}
...
function mapItem(item) {
  ...
  return {
    ...
    batchNo:
      item.batch_no ?? item.batchNo ?? meta.batchNo ?? meta.batch_no ?? "",
    mfgDate:
      normalizeDate(
        item.mfg_date ?? item.mfgDate ?? meta.mfgDate ?? meta.mfg_date,
      ) || "",
    expiryDate:
      normalizeDate(
        item.expiry_date ??
  ...
  try {
    ...
    if (scan != null) {
      ...
      if (product.ambiguous)
        return NextResponse.json({ error: product.message }, { status: 409 });
    ...
    if (id) {
      ...
      if (!row)
        return NextResponse.json(
          { error: "Remote GRN not found" },
          { status: 404 },
        );
      if (!canOpenRemoteGrnDraft(auth.user))
        return NextResponse.json(
          { error: "You do not have permission to open Remote GRN drafts" },
          { status: 403 },
        );
    ...
    return NextResponse.json({
      records: res.rows.map((row) => ({
        ...
        totalCost:
          canEditCp || canEditSp || canApprove
            ? toNumber(row.total_cost || row.items_cost_sum)
            : null,
        totalTax:
          canEditCp || canEditSp || canApprove ? toNumber(row.total_tax) : null,
    ...
    return NextResponse.json(
      { error: err.message || "Failed to load remote GRNs" },
      { status: 500 },
    );
  }
}
...
export async function PUT(request) {
  ...
  try {
    ...
    if (permissionCheck.error) return permissionCheck.error;
    const canEditCp =
      canEditRemoteGrnCp(auth.user) || canApproveRemoteGrn(auth.user);
    const canEditSp =
      canEditRemoteGrnSp(auth.user) || canApproveRemoteGrn(auth.user);

    const body = await request.json();
    const id = Number(body.id || body.remoteGrnId || 0) || null;
    if (!id)
      return NextResponse.json(
        { error: "Remote GRN id is required" },
        { status: 400 },
      );

    const destinationId =
      Number(body.destinationId || body.destination || body.storeId || 0) ||
      null;
    if (!destinationId)
    ...
    const existingByProduct = new Map(
      existingItemsRes.rows.map((row) => [Number(row.product_id), row]),
    );
    ...
    const normalizedItems = items.map((item) => {
      ...
      const taxValue =
        explicitTaxValue == null
          ? (qty * costPrice * taxRate) / 100
          : toNumber(explicitTaxValue);
      ...
      if (!productId || qty <= 0)
        throw new Error(
          "Every item needs product and quantity greater than zero",
        );
      if (costPrice < 0 || mrp < 0 || sellingPrice < 0)
        throw new Error("MRP, CP and SP cannot be negative");
      totalItems += qty;
    ...
    if (String(existing.status || "").toLowerCase() === "confirmed") {
      await client.query("ROLLBACK");
      return NextResponse.json(
        { error: "Confirmed Remote GRN cannot be edited" },
        { status: 409 },
      );
    }

    const workflowStatus = nextWorkflowStatus({
      user: auth.user,
    ...
    return NextResponse.json(
      { error: err.message || "Failed to update Remote GRN" },
      { status: 500 },
    );
  } finally {
...
export async function POST(request) {
  ...
  try {
    ...
    if (missing.length) {
      return NextResponse.json(
        { error: `Products not found in master: ${missing.join(", ")}` },
        { status: 422 },
      );
    }
    ...
    const normalizedItems = items.map((item) => {
      ...
      if (costPrice < 0 || mrp < 0 || sellingPrice < 0)
        throw new Error("MRP, CP and SP cannot be negative");
      if (!expiryDate)
        throw new Error("Expiry date is mandatory for every Remote GRN item");
      totalItems += qty;
      totalCost += qty * costPrice;
      totalTax += taxValue;
      return {
    ...
    const stockInRes = await client.query(
      `INSERT INTO stock_in (
         ...
         remarks, meta, created_by, created_at
       ) VALUES (
    ...
    await client.query(
      "UPDATE stock_in SET transaction_id = $1, reference_id = $1 WHERE id = $2",
      [transactionId, id],
    );
    ...
    return NextResponse.json(
      { error: err.message || "Failed to save Remote GRN" },
      { status: 500 },
    );
  } finally {
    if (client) client.release();
