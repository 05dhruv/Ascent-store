...
async function parseImportRequest(request) {
  ...
  const range = Number(formData.get("range") || 0);
  const selectedIndexes = new Set(
    parseJsonArray(formData.get("selectedIndexes"))
      .map(Number)
  ...
  const rows = selectedIndexes.size
    ? parsedRows.filter((row, index) => selectedIndexes.has(index))
    : parsedRows;
...
function normalizeImageUrl(value) {
  ...
  if (/^https?:\/\/\S+$/i.test(imageUrl) || /^data:image\/[a-z0-9.+-]+;base64,/i.test(imageUrl)) {
    return imageUrl;
  }
...
export async function POST(request) {
  try {
    ...
    const { type, rows } = await parseImportRequest(request); // type: 'categories' | 'sub-categories' | 'products'
