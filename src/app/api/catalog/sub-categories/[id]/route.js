...
export async function PUT(request, { params }) {
  try {
    ...
    await query(
      `DELETE FROM sub_category_products WHERE sub_category_id = $1`,
      [id],
    );
    ...
    if (err.code === "23505")
      return errorResponse("Sub Category already exists", 409);
    return errorResponse(err.message);
  }
}
...
export async function DELETE(request, { params }) {
  ...
  try {
    ...
    const linkedProducts = await client.query(
      `SELECT id FROM products WHERE sub_category_id = $1`,
      [id],
    );
    ...
    await client.query(
      `WITH target AS (
         ...
         WHERE r.table_name = 'sub_categories'
           AND r.resource_id = $1
           AND r.status = 'deleted'
         ORDER BY r.deleted_at DESC, r.id DESC
         LIMIT 1
       )
       UPDATE recycle_bin_items r
       SET metadata = COALESCE(r.metadata, '{}'::jsonb) || $2::jsonb
       FROM target
       WHERE r.id = target.id`,
      [
        String(id),
        JSON.stringify({
          affectedProductIds: linkedProducts.rows.map((row) => row.id),
        }),
      ],
    );
