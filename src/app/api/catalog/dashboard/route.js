...
export async function GET(request) {
  try {
    ...
    const pageSize = Math.min(100, Math.max(10, Number.parseInt(searchParams.get('pageSize') || '50', 10) || 50));
    const offset = (page - 1) * pageSize;
    const filter = String(searchParams.get('filter') || 'All').trim();
    const search = String(searchParams.get('search') || '').trim();
    const productListParams = [];
    const productListConditions = productScopeWhere
      ? [productScopeWhere.replace(/^WHERE\s+/i, '')]
      : [];
    const filterConditions = {
      'No image': `(p.image_url IS NULL OR p.image_url = '')`,
      'HSN missing': `(p.hsn_code IS NULL OR TRIM(p.hsn_code) = '')`,
      'Missing price': `COALESCE(p.mrp, 0) <= 0`,
      'Duplicate SKUs': `COALESCE(d.cnt, 0) > 1`,
      'Below cost': `COALESCE(p.cost_price, 0) > 0 AND COALESCE(p.mrp, 0) > 0 AND p.mrp < p.cost_price`,
      'Out of stock': `${ACTUAL_STOCK_EXPR} = 0`,
      'Needs attention': `(
        p.image_url IS NULL OR p.image_url = '' OR
        p.hsn_code IS NULL OR TRIM(p.hsn_code) = '' OR
        COALESCE(p.mrp, 0) <= 0 OR COALESCE(d.cnt, 0) > 1 OR
        (COALESCE(p.cost_price, 0) > 0 AND COALESCE(p.mrp, 0) > 0 AND p.mrp < p.cost_price) OR
        ${ACTUAL_STOCK_EXPR} = 0
      )`,
    };
    if (filterConditions[filter]) productListConditions.push(filterConditions[filter]);
    if (search) {
      productListParams.push(`%${search}%`);
      const searchParam = productListParams.length;
      productListConditions.push(`(
        p.name ILIKE $${searchParam} OR p.sku ILIKE $${searchParam} OR
        p.barcode ILIKE $${searchParam} OR p.hsn_code ILIKE $${searchParam}
      )`);
    }
    const productListWhere = productListConditions.length
      ? `WHERE ${productListConditions.join(' AND ')}`
      : '';
    ...
    return successResponse({
      ...
      pagination: {
        ...
        totalPages: Math.max(1, Math.ceil(filtered_total / pageSize)),
      },
    });
  } catch (err) {
