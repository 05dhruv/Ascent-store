...
function getDefaultRoute(role) {
  ...
  if (role === "user") return "/construction/projects";
...
export async function POST(request) {
  try {
    ...
    user.password_hash =
      freshPasswordResult.rows[0]?.password_hash || user.password_hash;
    ...
    debugLog(
      "[LOGIN] Fetching employee permissions (roles are informational only)",
    );
    let permissions = [];
    ...
    try {
      ...
      console.warn(
        "[LOGIN] Failed to fetch employee permissions:",
        err.message,
      );
    }
    ...
    try {
      ...
      console.warn(
        "[LOGIN] user_stores table does not exist or query failed:",
        err.message,
      );
    ...
    return NextResponse.json(
      {
        ...
        message:
          "We could not complete sign-in right now. Please try again or contact the administrator.",
      },
      { status: 500 },
    );
  }
