...
export async function PUT(request, { params }) {
  try {
    ...
    if (!isMinimalStore && !locationType)
      return errorResponse("Location type is required", 422);
    if (!addressLine1) return errorResponse("Address line 1 is required", 422);
    if (!isMinimalStore && !city) return errorResponse("City is required", 422);
    if (!isMinimalStore && !state)
      return errorResponse("State is required", 422);
    if (!isMinimalStore && !pincode)
      return errorResponse("Pincode is required", 422);
    if (!isMinimalStore && !/^\d{6}$/.test(pincode))
      return errorResponse("Pincode must be 6 digits", 422);
    if (!isMinimalStore && !country)
      return errorResponse("Country is required", 422);
    ...
    const deliveryErrors = isMinimalStore
      ? []
      : validateStoreDeliveryPayload(body);
