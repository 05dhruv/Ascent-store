...
export async function GET(request) {
  ...
  try {
    ...
    const store=Number(sp.get('store'))||null, project=Number(sp.get('project'))||null;
    ...
    const params=[scope,store,project,`%${sp.get('search')||''}%`,sp.get('from')||null,sp.get('to')||null];
    const transfers=await query(`SELECT t.id,t.transaction_id,t.source_id,t.destination_id,t.status,t.workflow_version,
      ...
      AND ($5::date IS NULL OR t.created_at >= $5::date) AND ($6::date IS NULL OR t.created_at < $6::date+INTERVAL '1 day')
    ...
    const buckets=await query(`SELECT b.store_id,s.name location,p.id product_id,p.name product,p.sku,p.unit,
      SUM(CASE WHEN b.status='active' THEN b.available_qty-b.reserved_qty ELSE 0 END) available,
      SUM(b.reserved_qty) reserved,
      SUM(CASE WHEN b.status IN ('quarantine','blocked') THEN b.available_qty ELSE 0 END) quarantine,
      SUM(CASE WHEN b.status='damaged' THEN b.available_qty ELSE 0 END) damaged,
      SUM(CASE WHEN b.status='rejected' THEN b.available_qty ELSE 0 END) rejected,
    ...
    const ledger=await query(`WITH movements AS (
      ...
      AND ($5::date IS NULL OR m.created_at >= $5::date) AND ($6::date IS NULL OR m.created_at < $6::date+INTERVAL '1 day')
