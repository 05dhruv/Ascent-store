...
export async function POST(request, { params }) {
  const { id } = await params;
    ...
    try {
      ...
      const draft = await client.query('SELECT id, status, method, COALESCE(source_id, destination_id) AS source_id, destination_id, transaction_id FROM stock_out WHERE id = $1 FOR UPDATE', [id]);
      ...
      if (draft.rows[0].method === 'return_warehouse') {
        await client.query('ROLLBACK');
        return NextResponse.json({ error: 'Use Stock Transfer for returns to an internal warehouse' }, { status: 403 });
      }
      const storeCheck = requireStore(auth.user, draft.rows[0].source_id);
      if (storeCheck.error) {
        await client.query('ROLLBACK');
