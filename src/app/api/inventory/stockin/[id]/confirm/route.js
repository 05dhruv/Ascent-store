...
import { inspectedCounts, receiveInspectedStock } from '@/lib/grnInspection';
...
import { sendVendorPaymentNotification } from "@/lib/email";
...
function normalizeBatchRows(item) {
  ...
  if (!rawBatches.length) {
    return [
      {
        qty: fallbackQty,
        damaged_qty: item.damaged_qty, rejected_qty: item.rejected_qty,
  ...
  return rawBatches
    .map((batch) => ({
      ...
      damaged_qty: batch.damaged_qty, rejected_qty: batch.rejected_qty,
  ...
  try {
    ...
    if (isRemoteGrn) {
      ...
      const remoteStatus = String(
        stockInRow.rows[0].status || "",
      ).toLowerCase();
      if (remoteStatus !== "ready_for_superadmin") {
        return NextResponse.json(
          {
            error:
              "Remote GRN must complete CP and SP review before super admin approval",
          },
          { status: 409 },
        );
      }
    ...
    if (isStoreDestination && !isDirectVendorReceipt) {
      return NextResponse.json({error:'Internal warehouse/site receipts must use Stock Transfer and destination receipt.'},{status:400});
    }
    if (form.inspectionConfirmed !== true || !String(form.checkedBy || '').trim() || !String(form.receiptEvidence || '').trim()) {
      return NextResponse.json({error:'Complete physical inspection, checked-by and receipt proof before confirming GRN.'},{status:400});
    }
    for (const item of items) for (const batch of normalizeBatchRows(item)) inspectedCounts(batch);
    ...
    try {
      ...
      for (const item of items) {
        ...
        const approvalRequest = isRemoteGrn
          ? null
          : await createMarginApprovalRequest(client, {
              stockInId: id,
              stockInItemId: null,
        ...
        return NextResponse.json(
          {
            ...
            message:
              "Stock in is on hold until margin approval is completed. After pricing approval, open this GRN and confirm the inspected quantities.",
            id,
            marginApprovalCount,
      ...
      for (const item of items) {
        ...
        if (isStoreDestination && !isVendorToStoreReceipt) {
          ...
          for (const batch of batchRows) {
            ...
            await receiveInspectedStock(client, {
              inspection: inspectedCounts(batch),
              stockInId: id,
              stockInItemId: stockInItemRes.rows[0]?.id,
              productId: pid,
              storeId: destinationId,
              ...
              meta: {
                actorId: auth.user.id, transactionId: stockInRow.rows[0].transaction_id,
                productName,
                invoiceNumber: form.invoice_number || null,
                costPrice,
                mrp,
      ...
      const vendorInfoRes = await client.query(
        `SELECT email, name FROM vendors WHERE id = $1`,
        [vendorId],
      );
      const vendorEmail = vendorInfoRes.rows[0]?.email;
      const vendorName = vendorInfoRes.rows[0]?.name;
      ...
      await client.query(
        ...
        [
          ...
          totalCost,
          totalTax,
          JSON.stringify({ ...form, workflowVersion: 2, receivedBy: auth.user.id }),
          id,
        ],
      );

      // Every confirmed vendor GRN must have a payable invoice, including
      // direct/non-PO stock-ins where the vendor was selected by ID.
      if (vendorId) {
        ...
        if (purchaseOrderId) {
          const poTerms = await client.query(
            `SELECT payment_due_date, vendor_credit_days,
                    (created_at AT TIME ZONE 'Asia/Kolkata')::date AS created_date
             FROM purchase_orders WHERE id = $1`,
            [purchaseOrderId],
          );
          linkedPurchaseOrder = poTerms.rows[0] || null;
        }
        paymentTerms = await resolveVendorPaymentTerms(client, {
          vendorId,
          baseDate: linkedPurchaseOrder?.created_date || invoiceDate,
          paymentDueDate: linkedPurchaseOrder?.payment_due_date,
        ...
        if (existingInvoiceRes.rows[0]) {
          ...
          const invoiceRes = await client.query(
            `INSERT INTO vendor_invoices (
              ...
              invoice_date, due_date, created_by, remarks, status, meta, created_at, updated_at
            ) VALUES ($1, $2, $3, $4, $5, 0, $6, $7, $8, $9, 'Pending', $10::jsonb, NOW(), NOW())
            RETURNING id`,
            [
              vendorId,
              purchaseOrderId,
      ...
      await client.query(`COMMIT`);
      let emailNotification = { sent: false, reason: "Vendor email or payment terms are unavailable" };
      if (vendorEmail && paymentTerms) {
        try {
          emailNotification = await sendVendorPaymentNotification({
            vendorEmail,
            vendorName: vendorName || "Vendor",
            paymentDueDate: paymentTerms.paymentDueDate,
            creditDays: paymentTerms.creditDays,
            grnNumber: stockIn.transaction_id || `GRN-${id}`,
            invoiceNumber: form.invoice_number || stockIn.invoice_number,
            amount: grossInvoiceAmount,
          });
        } catch (emailError) {
          // Email is a post-confirmation notification. A mail-server issue
          // must never roll back or falsely report a failed GRN confirmation.
          console.error("[stockin confirm] GRN email failed:", emailError.message);
