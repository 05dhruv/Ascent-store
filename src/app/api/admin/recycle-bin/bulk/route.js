...
export async function POST(request) {
  ...
  try {
    ...
    if (!ids.length)
      return validationError({ ids: "Select at least one recycle-bin record" });
    if (!["restore", "purge"].includes(action)) {
      return validationError({
        action: "Bulk action must be restore or purge",
      });
    }
    ...
    for (const id of ids) {
      if (action === "restore") {
        ...
        results.push(
          item ? { id, purged: item.purgedCount || 1 } : { id, skipped: true },
        );
      }
    }
    ...
    return successResponse(
      ...
      action === "restore"
        ? "Selected recycle-bin records restored"
        : "Selected recycle-bin records deleted",
    );
  } catch (err) {
    ...
    return errorResponse(
      err.message || "Failed to run bulk recycle-bin action",
    );
  } finally {
    client.release();
  }
