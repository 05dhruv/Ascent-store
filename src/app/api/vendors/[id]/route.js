...
async function saveVendorBrands(db, vendorId, brandIds) {
  await db.query("DELETE FROM vendor_brands WHERE vendor_id = $1", [vendorId]);
...
function normalizePayload(body) {
  ...
  if (!String(body.gst_number || "").trim())
  ...
  if (!Number.isInteger(creditDays) || creditDays < 1 || creditDays > 3650)
    throw new Error("Credit days must be a whole number between 1 and 3650");
...
export async function PUT(request, context) {
  ...
  try {
    ...
    await setRecycleBinContext(client, auth.user.id, "Vendor brands updated");
    ...
    await saveVendorBrands(client, id, payload.brand_ids);
    await client.query("COMMIT");
    return NextResponse.json(
      mapVendor({ ...res.rows[0], brand_ids: payload.brand_ids }),
    );
  } catch (err) {
    ...
    if (err.code === "23505") {
      return NextResponse.json(
        { error: "Vendor with same mobile or email already exists" },
        { status: 409 },
      );
    }
    if (
      /required|exactly 10 digits|valid email|Credit days|phone/i.test(
        String(err.message || ""),
      )
    ) {
...
export async function DELETE(request, context) {
  ...
  try {
    ...
    if (
      Number(counts.purchase_orders || 0) > 0 ||
      Number(counts.vendor_invoices || 0) > 0
    ) {
      await query(
