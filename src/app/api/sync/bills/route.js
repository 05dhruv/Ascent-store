...
export async function POST(request) {
  ...
  for (const bill of bills) {
    ...
    try {
      ...
      const savedBillNumber = billResult.rows[0].bill_number;
      ...
      if (items.length > 0) {
        ...
        const soResult = await query(
          `INSERT INTO stock_out (
           ...
           RETURNING id`,
          [stockOutTxId, billStoreId, savedBillNumber, totalQty, billCreatedAt],
        );
        ...
        for (const item of items) {
          const allocations = await allocateBatchStock(
            { query },
            {
              ...
              meta: { syncId, billNumber: savedBillNumber },
            },
          );
