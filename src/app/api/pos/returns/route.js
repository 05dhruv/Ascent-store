...
function canReviewReturn(user, returnRow) {
  return (
    isSuperAdmin(user) ||
    isStoreAdmin(user, returnRow?.store_id) ||
    canApproveStoreReturnExchange(user, returnRow) ||
    canProcessStoreExchange(user, returnRow)
  );
}
  ...
  try {
    ...
    const exchangeOnly =
      ...
      !user.permissions?.some((permission) =>
        ["CREATE_POS_BILL", "MANAGE_BILLING", "MANAGE_ORDERS", "*"].includes(
          permission,
        ),
      );
    ...
    for (const item of items) {
      ...
      requestedByProduct.set(
        productId,
        roundQty((requestedByProduct.get(productId) || 0) + qty),
      );
    }
    ...
    const soldByProduct = new Map(
      soldQtyRes.rows.map((row) => [Number(row.product_id), row]),
    );
...
export async function PATCH(req) {
  try {
    ...
    if (!canReviewReturn(auth.user, returnRow)) {
      return errorResponse(
        "Only this store admin, approved employee, or super admin can review this request",
        403,
      );
    }
