...
function normalizeSessionRow(row) {
  return {
    ...
    handoverStatus:
      row.handover_status || (row.closed_at ? "pending_handover" : "active"),
...
export async function GET(request) {
  try {
    ...
    const storeId = Number(
      url.searchParams.get("storeId") || url.searchParams.get("store_id") || 0,
    );
    ...
    if (dateTo) {
      params.push(dateTo);
      whereClauses.push(
        `ucs.session_start_at < ($${params.length}::date + INTERVAL '1 day')`,
      );
    }
    ...
    return NextResponse.json(
      { error: err.message || "Failed to fetch user counter sessions" },
      { status: 500 },
    );
  }
}
  ...
  try {
    ...
    const storeId =
      body.storeId || body.store_id
        ? Number(body.storeId || body.store_id)
        : null;
    const counterId =
      body.counterId || body.counter_id
        ? Number(body.counterId || body.counter_id)
        : null;
    const deviceId =
      body.deviceId || body.device_id
        ? Number(body.deviceId || body.device_id)
        : null;
    const serialNumber = String(
      body.serialNumber || body.serial_number || "",
    ).trim();
    const counterName = String(
      body.counterName || body.counter_name || "",
    ).trim();
    const deviceUid = String(body.deviceUid || body.device_uid || "").trim();
    const counterUid = String(
      body.counterUid || body.counter_uid || counterName || "",
    ).trim();
    let openingCash = Number(body.openingCash ?? body.opening_cash);
    ...
    if (
      ...
      return NextResponse.json(
        { error: "You can only open your own POS session" },
        { status: 403 },
      );
    }
    ...
    if (deviceUid || counterUid) {
      ...
      if (activeSession.rows.length > 0) {
        return NextResponse.json(normalizeSessionRow(activeSession.rows[0]), {
          status: 200,
        });
      }

      // The phone may have discarded localStorage while the server-side shift
      // remains active. Resume that shift rather than creating a duplicate one.
      const resumableSession = await query(
        `SELECT ucs.id, ucs.user_id, ucs.counter_id, ucs.device_id, ucs.store_id,
                ucs.session_id, ucs.session_start_at, ucs.session_end_at, ucs.is_active,
                ucs.serial_number, ucs.counter_name, ucs.device_uid, ucs.counter_uid, ucs.meta,
                s.name AS store_name, u.name AS user_name
      ...
      if (resumableSession.rows.length > 0) {
        return NextResponse.json(
          normalizeSessionRow(resumableSession.rows[0]),
          { status: 200 },
        );
      }
    }
    ...
    const storeActivity = await client.query(
      `SELECT
         EXISTS (
           SELECT 1
           FROM sales_bills
         ...
         EXISTS (
           ...
           WHERE store_id = $1
             AND amount > 0
             AND transaction_type <> 'opening_float'
           LIMIT 1
         ) AS has_cash_activity`,
      [storeId],
    );
    const canOpenZeroCashSession =
      storeBalance.currentCash === 0 &&
      !storeActivity.rows[0]?.has_sales &&
      !storeActivity.rows[0]?.has_cash_activity;

    if (openingCash > 0 && openingCash > storeBalance.currentCash) {
      if (canOpenZeroCashSession) {
        openingCash = 0;
      } else {
        await client.query("ROLLBACK");
        return NextResponse.json(
          {
            error: `Only ${storeBalance.currentCash} cash is available for opening float`,
          },
          { status: 400 },
        );
      }
    ...
    // Super Admin can open a session without an opening float. This is useful
    // for supervision, testing and stores where no cash drawer is used.
    const allowsZeroOpeningCash =
      openingCash === 0 &&
      (canOpenZeroCashSession || auth.user.role === "super_admin");

    if (openingCash === 0 && !allowsZeroOpeningCash) {
      await client.query("ROLLBACK");
      return NextResponse.json(
        {
          error:
            "Opening cash must be greater than zero for subsequent sessions",
        },
        { status: 400 },
      );
    }
    ...
    const nextStoreCash = Number(
      (storeBalance.currentCash - openingCash).toFixed(2),
    );
    await client.query(
    ...
    return NextResponse.json(normalizeSessionRow(result.rows[0]), {
      status: 201,
    });
  } catch (err) {
    ...
    return NextResponse.json(
      { error: "Failed to open user counter session" },
      { status: 500 },
    );
  } finally {
