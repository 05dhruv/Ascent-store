...
export default function EditStorePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isMinimalStore && Number(form.storeAreaSqFt || 0) < 600) {
    ...
    if (!isMinimalStore && Number(form.costPerSqFt || 0) < MIN_COST_PER_SQ_FT) {
    ...
    for (const field of DOCUMENT_FIELDS) {
      const isRequired =
        ...
        !(
          field.key === "agreement" &&
          ["COCO", "FOCO"].includes(form.franchiseType)
        );
    ...
    if (!isMinimalStore && !/^\d{6}$/.test(String(form.pincode || "").trim())) {
      setError("Pincode must be 6 digits");
      return;
    }
    if (!isMinimalStore && !/^\d{10}$/.test(form.managerMobile)) {
      setError("Mobile number must be exactly 10 digits");
      setFieldErrors({
        managerMobile: "Mobile number must be exactly 10 digits",
      });
      return;
    }
    if (
