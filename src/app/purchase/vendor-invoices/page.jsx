...
export default function VendorInvoicesPage() {
  ...
  return (
    <MainLayout>
      ...
      <div className="flex flex-none items-start justify-between gap-4 mb-5">
        <div>
      ...
      <div className="flex-none bg-white rounded-xl border border-gray-200 p-4 mb-5 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
                  ...
                  <tr key={rowIdx} className="border-b border-gray-100 hover:bg-blue-50/50 transition-colors">
                    {tableHeaders.map((header, colIdx) => (
                      <td key={colIdx} className="px-4 py-3 text-[13px] text-gray-700">
                        {header === 'Actions' ? (
                          <button
                            onClick={() => openSettlement(paginatedRecords[rowIdx])}
                            disabled={Number(paginatedRecords[rowIdx]?.amountLeft || 0) <= 0}
      ...
      {activeInvoice && typeof document !== 'undefined' && createPortal(
