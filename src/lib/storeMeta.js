...
function cleanInteriorItems(items) {
  ...
  for (const key of INTERIOR_ITEM_KEYS) {
    ...
    if (key === "racks") {
      ...
      cleanItem.children = {};
      const children =
        item.children && typeof item.children === "object" ? item.children : {};
      let childSubtotal = 0;
      for (const childKey of RACK_INTERIOR_ITEM_KEYS) {
        ...
        const childTotal = childEnabled
          ? Math.round(childAmount * childUnits * 100) / 100
          : 0;
