...
import {
  ...
  getRecycleRestorePriority,
} from "@/lib/recycleBinSchema";
...
function getAffectedProductIds(metadata) {
  if (!Array.isArray(metadata?.affectedProductIds)) return [];
  return metadata.affectedProductIds
    .map(Number)
    .filter((id) => Number.isFinite(id) && id > 0);
}

async function relinkProductsAfterRestore(client, row) {
  const restoredId = Number(row.deleted_snapshot?.id || row.resource_id);
  if (!Number.isFinite(restoredId) || restoredId <= 0) return 0;

  const affectedProductIds = getAffectedProductIds(row.metadata);
  if (!affectedProductIds.length) return 0;
  ...
  if (row.table_name === "categories") {
    const result = await client.query(
      ...
      [restoredId, affectedProductIds],
    );
    return result.rowCount || 0;
  }

  if (row.table_name === "sub_categories") {
    const result = await client.query(
      `UPDATE products
...
export async function setRecycleBinContext(
  client,
  userId,
  reason = "",
  ...
  await client.query(
    `SELECT set_config('app.recycle_bin_operation_id', $1, true)`,
    [operationId],
  );
  await client.query(`SELECT set_config('app.recycle_bin_reason', $1, true)`, [
    String(reason || ""),
  ]);
  return operationId;
}
...
export async function restoreRecycleBinOperation(client, itemId, user) {
  ...
  const rows = related.rows.sort(
    (a, b) =>
      getRecycleRestorePriority(a.table_name) -
        getRecycleRestorePriority(b.table_name) || Number(a.id) - Number(b.id),
  );
  ...
  for (const row of rows) {
    ...
    const relinkedProductCount = await relinkProductsAfterRestore(client, row);
