...
export async function parseBulkSheet(file) {
  ...
  // Keep Excel dates as serial numbers so we can read their calendar parts
  // directly. Converting them to JavaScript Date objects here can shift a day
  // when workbook and browser time zones differ.
...
function normalizeCellValue(value, cell = null, XLSX = null) {
  // Prefer Excel's underlying date serial/value. Excel can display a date as
  // m/d/yy based on the workstation locale even when the user entered dd/mm/yy.
  // Reading the display text first would turn 13/02/27 into 2/13/27 and reject it.
  const dateValue = getExcelDateValue(value, cell, XLSX);
  if (dateValue) return dateValue;

  const displayed = String(cell?.w || "").trim();
  if (displayed && /^\d{1,2}[\/\-.]\d{1,2}[\/\-.]\d{2,4}$/.test(displayed)) {
    return displayed;
  }
