...
const CREATE_SALES_BILL_ITEMS_SQL = `
  CREATE TABLE IF NOT EXISTS sales_bill_items (
    ...
    is_promotion_free BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  );
`;
...
const MIGRATE_SALES_BILLING_SQL = `
  ...
  ALTER TABLE sales_bill_items
    ...
    ADD COLUMN IF NOT EXISTS is_promotion_free BOOLEAN NOT NULL DEFAULT FALSE,
    ADD COLUMN IF NOT EXISTS batch_allocations JSONB NOT NULL DEFAULT '[]'::jsonb,
    ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT NOW();

  CREATE INDEX IF NOT EXISTS idx_sales_bill_items_promotion_free
    ON sales_bill_items (promotion_id, is_promotion_free)
    WHERE is_promotion_free = TRUE;
