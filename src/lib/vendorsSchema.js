...
export const ensureVendorsSchema = makeSchemaEnsurer('vendors', SCHEMA_VERSION, async () => {
  await query(`
    ...
    BEGIN
      IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'vendors_credit_days_check'
          AND conrelid = 'vendors'::regclass
      ) THEN
        ALTER TABLE vendors ADD CONSTRAINT vendors_credit_days_check
          CHECK (credit_days IS NULL OR (credit_days BETWEEN 1 AND 3650));
      END IF;
    END
    $migration$;
